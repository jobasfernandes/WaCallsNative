...
// noopSocket is a do-nothing signaling.Socket. Only OwnLID matters here: the
...
var _ signaling.Socket = (*noopSocket)(nil)
...
func TestStartCallTerminatesAnOfferThatReachedTheWire(t *testing.T) {
	...
	err := m.StartCall(context.Background(), "CID",
		types.JID{User: "peer", Server: types.HiddenUserServer})
...
func TestStartCallSendsNoTerminateWhenTheOfferNeverLeft(t *testing.T) {
	...
	if err := m.StartCall(context.Background(), "CID",
		types.JID{User: "peer", Server: types.HiddenUserServer}); err == nil {
