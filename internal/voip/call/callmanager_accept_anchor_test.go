...
// stubSocket is a no-op signaling.Socket. Only the identity accessors matter
// here: HandleCallAccept reaches the socket through ownCredJid() when it
// derives the SRTP keys, and sends transport stanzas we do not assert on.
...
var _ signaling.Socket = (*stubSocket)(nil)

// acceptStanza builds the minimal <call><accept/></call> stanza
// HandleCallAccept parses. DecryptCallKey returns nil, so the call-key
// decryption branch is skipped and the test stays focused on the anchoring.
func acceptStanza(callID string) *waBinary.Node {
...
// the SRTP keys are derived from. The existing guard at the top of
// HandleCallAccept already anchors the media path to the first accepting
// device; this test locks the invariant.
func TestSrtpKeysStayAnchoredToTheFirstAcceptingDevice(t *testing.T) {
	...
	m.HandleCallAccept(context.Background(), acceptStanza(callID), first)
	...
	srtpAfterFirst := m.srtp
	if srtpAfterFirst == nil {
		t.Fatal("first accept should have initialised the SRTP manager")
	}

	// Second companion device accepts the same offer.
	second := types.JID{User: "peer", Device: 2, Server: types.HiddenUserServer}
	m.HandleCallAccept(context.Background(), acceptStanza(callID), second)
	...
	if m.srtp != srtpAfterFirst {
		t.Fatal("a later accept replaced the live SRTP manager")
	}
}
...
func TestFirstAcceptStillAnchorsThePeer(t *testing.T) {
	...
	m.HandleCallAccept(context.Background(), acceptStanza(callID), peer)
